...
const findLastFacultyId = async () => {
    ...
    return lastFaculty ? lastFaculty.id ? lastFaculty.id : undefined : undefined
}
const findLastAdminId = async () => {
    const lastAdmin: any = await User.findOne({
        role: "admin",
    }, { id: 1, _id: 0 }).sort({ createdAt: -1 }).lean();
    return lastAdmin ? lastAdmin.id ? lastAdmin.id : undefined : undefined
}
...
export const generateFacultyId = async () => {
    ...
    const incrementedId = (Number(currentId) + 1);
    const newId = incrementedId.toString().padStart(4, "0");
    return newId
}
export const generateAdminId = async () => {
    const currentId = await findLastAdminId() || (0).toString().padStart(3, "0");
    const incrementedId = (Number(currentId) + 1);
    const newId = incrementedId.toString().padStart(3, "0");
    return newId
}